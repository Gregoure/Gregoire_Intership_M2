...
### We state here the possible alpha values for the start of simulation
seq = np.arange(0.01,0.45,0.01)
...
def SetMetapop(nbsite, taillepop): #Creates sites objects containing populations
    ...
    for i in range(nbsite): # Creates sites, the 1st will always contain one infected and the other 0
        if i == 0:
            newsite = classes.Site(effectifS=400, effectifI=100) # Anciennement S = 30 et I = 55
            newsite.Index = i
            ...
            for j in range(newsite.effectifI):
                ...
                #sampled_trait = float(np.random.normal(0.15,0.01,1))
                #print(sampled_trait, "COUCOU")
                #newsite.traitvalues.append(round(sampled_trait,3)) # For random sample from a given law, here normal

                randomseq = round(np.random.choice(seq), 2)   # Random assignation of virulence values to each individuals
                newsite.traitvalues.append(randomseq)

                #newsite.traitvalues.append(0.2) # (0.15) For sampling from predefined trait vector
            ...
            for j in range(newsite.effectifI):
                ...
                #sampled_trait = float(np.random.normal(0.15, 0.01, 1))
                #newsite.traitvalues.append(round(sampled_trait, 3))  # For random sample from a given law, here normal
                randomseq = round(np.random.choice(seq), 2)
                newsite.traitvalues.append(randomseq)
