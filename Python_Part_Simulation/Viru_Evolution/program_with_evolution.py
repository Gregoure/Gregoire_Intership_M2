...
mu = Params.mu # Per capita natural death rate
...
def RunModel(seed, paramSU, paramINF) :
    ...
    dirfile = "dS=" + str(paramSU) + '_' + "dI=" + str(paramINF)
    rep_actuel = "D:/gaze/donnees/bureau/Stage M2/Dispersal-2-modelisation_dev/" + dirfile  # Change directory
    ...
    if dirtrue == False:
        ...
        os.mkdir(rep_actuel + "/Distributions")

        os.mkdir(rep_actuel + "/Traits")

    # Creation of three folders for each type of file
    rep_densities = rep_actuel + "/Densities"
    rep_distrib = rep_actuel + "/Distributions"
    rep_traits = rep_actuel + "/Traits"
    ...
    DeathS = classes.Event(name='Death S',propensity='mu*self.S', Schange='-1', Ichange='0', order=3,EvolvingTrait=Evoltrait)
    ...
    DeathI = classes.Event(name='Death I',propensity='(mu + alpha) *self.I', Schange='0', Ichange='-1', order=1,EvolvingTrait=Evoltrait)
    ...
    datadensity.to_csv(os.path.join(rep_densities, 'Metapop_outputs_ESSfigure_0811' + '_' + str(dS) + '-' + str(dI) + '_' + str(seed) + '.csv'))
    ...
    datatrait.to_csv(os.path.join(rep_traits,'Traits_outputs_ESSfigure_0811' + '_' + str(dS) + '-' + str(dI) + '_' + str(seed) + '.csv'))
    ...
    datadistrib.to_csv(os.path.join(rep_distrib,'Distribution_outputs_ESSfigure_0811' + '_' + str(dS) + '-' + str(dI) + '_' + str(seed) + '.csv'))
...
### Values of dS and dI to fixed for simulation
list_paramsS = [0.1]
list_paramsI = [1.1]



#list_config =["Island"]l
